...
import { MapPin, Briefcase, Building2, Linkedin } from 'lucide-react'
...
export default function UserCard({ profile, currentUserId }: UserCardProps) {
  ...
  return (
    <Card className="group hover:shadow-2xl transition-all duration-300 border-0 overflow-hidden bg-white shadow-md hover:shadow-xl rounded-2xl">
      <Link to={`/profile/${profile.id}`} className="block">
        <div className="pt-8 pb-5 px-6 flex flex-col items-center relative bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
          ...
          <div className="text-center space-y-1.5">
            <h3 className="font-bold text-xl text-gray-900 leading-tight">{profile.full_name}</h3>
            <p className="text-sm text-gray-500 font-medium">{profile.age} years old</p>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 pt-5 space-y-4">
          <div className="space-y-3">
            <div className="flex items-center justify-center gap-2.5 text-sm">
              ...
              <span className="text-gray-800 font-semibold leading-relaxed text-center">{profile.job_title}</span>
            </div>
            <div className="flex items-center justify-center gap-2.5 text-sm">
              <div className="p-1.5 bg-purple-100 rounded-lg">
                <Building2 className="h-4 w-4 text-purple-600 flex-shrink-0" />
              </div>
              <span className="text-gray-700 leading-relaxed text-center font-medium">{profile.company}</span>
            </div>
            <div className="flex items-center justify-center gap-2.5 text-sm">
              <div className="p-1.5 bg-pink-100 rounded-lg">
                <MapPin className="h-4 w-4 text-pink-600 flex-shrink-0" />
              </div>
              <span className="text-gray-700 leading-relaxed text-center font-medium">{profile.city}, {profile.state}</span>
            </div>
          </div>

          {profile.bio && (
            <p className="text-sm text-gray-600 leading-relaxed line-clamp-2 text-center pt-2">
      ...
      <div className="px-6 pb-6 pt-0 space-y-3">
        {profile.linkedin_profile_url && (
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-3 px-4 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors text-sm font-semibold text-indigo-600 hover:text-purple-600"
          >
